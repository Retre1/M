...
> ⚠️ **Метрики ниже недействительны.** `sharpe` и `profit_factor` во всех шести
> прогонах считались по ряду наград эпизодов, а не по реализованному PnL.
> `PF = 0` — арифметическое тождество, а не поведение агента; ряд «best Sharpe»
> −147 → −58 → −52 несопоставим между прогонами из-за перемасштабирования
> награды. Разбор: [[../analysis/metrics-invalidation|Анализ: метрики прогонов
> 1–6 недействительны]]. Фактические выводы о поведении агента читайте только
> по OOS-бэктестам.
